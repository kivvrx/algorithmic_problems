...
//s--(3)-- > v--(2)-- > to
//Прямой путь : s → to = 10.
//Через v : s → v → to = 3 + 2 = 5.
//Когда мы обрабатываем v, возникает вопрос :
//dist[s][to](10) > dist[s][v](3) + w(2) ?
//Да, 10 > 5 → обновляем.Без этой проверки мы бы так и оставили 10, хотя правильный ответ — 5.
